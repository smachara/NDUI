...
namespace smachara\GeneratorBundle\Command;

use Doctrine\ORM\ORMException;
use smachara\GeneratorBundle\Generator\DoctrineCrudGenerator;
use smachara\GeneratorBundle\Generator\DoctrineFixturesGenerator;
use smachara\GeneratorBundle\Generator\DoctrineFormGenerator;
    ...
    protected function configure()
    {
        $this
            ->setDefinition([
                ...
                new InputOption('with-security', '', InputOption::VALUE_NONE, 'Whether or not to generate feature-specific roles'),
            ])
...
Using the --with-security option allows to generate feature-specific roles annotation for each actions.

<info>php %command.full_name% --entity=AppBundle:Post --route-prefix=post_admin --with-security</info>
...
EOT
            ...
            ->setName('s7:generate:crud')
            ->setAliases(['generate:s7:crud']);
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $withSecurity = $input->getOption('with-security');
        ...
        $generator->generate($bundle, $entity, $metadata[0], $format, $prefix, $withWrite, $withSecurity, $forceOverwrite, $layout, $bodyBlock, $usePaginator, $theme, $withFilter, $withSort);
    ...
    protected function getSkeletonDirs(BundleInterface $bundle = null)
    {
        ...
        if (isset($bundle) && is_dir($dir = $bundle->getPath().'/Resources/smacharaGeneratorBundle/skeleton')) {
            $skeletonDirs[] = $dir;
        }

        if (is_dir($dir = $this->getContainer()->get('kernel')->getRootdir().'/Resources/smacharaGeneratorBundle/skeleton')) {
            $skeletonDirs[] = $dir;
        }

        $bundlesDirectories = $this->getContainer()->get('kernel')->locateResource('@smacharaGeneratorBundle/Resources/skeleton', null, false);
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        //Secutiy anotation?  strategy feature-specific roles
        $withSecurity = $input->getOption('with-write') ?: false;
        $output->writeln([
            '',
            'We use feature-specific roles strategy. That means,each action of a controller have an @Secutity Annotation.',
            'The format of the role is ROLE_ENTITY_ACTION',
            '',
        ]);
        $question = new ConfirmationQuestion($questionHelper->getQuestion('Do you want to generate the "Security Annotation" for each actions', $withSecurity ? 'yes' : 'no', '?', $withSecurity), $withSecurity);
        $input->setOption('with-security', $helper->ask($input, $output, $question) != false);
